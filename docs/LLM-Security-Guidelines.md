...
### 🔴 CRITICAL: Enable Privacy Mode

**Enable Privacy Mode before using Cursor on any non-public code.**

Privacy Mode enforces Zero Data Retention (ZDR) with Cursor's model providers — your code is not stored or used for training. Without it, prompts and code snippets may be retained.

- **Individual accounts**: Cursor Settings → General → Privacy Mode → Enable
- **Team accounts**: Enabled by default; verify in your team settings
...
If you want to exclude large generated files or vendor directories from codebase search without blocking AI access to them, use `.cursorindexingignore` instead. Files listed here are excluded from indexing but remain accessible to AI features.
...
### 🟠 HIGH: Cursor Rules Security

Cursor Rules (`.cursor/rules/`) are version-controlled files included at the start of every AI prompt. Because they are sent to the model on every session:

- **Never put secrets, tokens, API keys, or internal URLs** in rules files
- Treat rules files as public documentation — they should contain only patterns, conventions, and non-sensitive architectural guidance
- Review rules files before committing, especially if generated by AI

### 🟠 HIGH: MCP Server Security

MCP servers execute code on your behalf and can access external services. They represent a significant attack surface.

**Before installing any MCP server**:
- Only install from trusted, audited sources
- Review the server's source code for critical integrations
- Check what files, APIs, and credentials it can access

**Configuration**:
- Use `${env:NAME}` syntax for secrets — never hardcode credentials in MCP config
- Prefer `stdio` (local) transport over remote HTTP/SSE endpoints for sensitive integrations
- Use API keys with minimal required permissions
- Consider running servers in isolated environments for highly sensitive data

**Remember**: `.cursorignore` does not protect secrets from MCP servers — a server with file system access can read any file the process can reach.
